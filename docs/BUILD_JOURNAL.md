...
## Nav-rail polish (2026-06-14)

A cluster of small surface fixes. The standing ring on the dashboard was
drawn with an offset start angle that made the gauge read wrong; corrected
to start at twelve o'clock. The timer scales with the window now instead of
sitting fixed. The nav rail was reorganised and given a slide animation when
it opens and closes, and the bilingual labels (JP + EN) in the rail and the
agenda were nudged onto a single shared baseline so they stop looking ragged.

---

## Hardening pass (2026-06-15)

Before piling on more features, a safety sweep. Native notifications shell
out to `osascript` / `notify-send`, so any user text that reaches them
(deadline titles, task names) is now escaped rather than interpolated raw —
no more breaking the command, or worse, with a stray quote. The `.ics`
importer already capped file size; the cap got tightened and the read stays
bounded. And the `Tmds.DBus.Protocol` dependency was pinned to a patched
**0.21.3** (a backported fix that keeps the 0.21.x line Avalonia expects),
so Linux notifications don't drag in the vulnerable 0.20.0 transitively.

---

## Recall and reflection (2026-06-15)

The big feature drop. **Flashcards**: a 復習 · review destination with
spaced-repetition decks. `Deck` holds `Flashcard`s, `ReviewScheduler` owns
the recall logic (grade a card again / hard / good / easy and it reschedules
by how well it went), and the review queue pulls what's due across decks.
**Study goals** and **per-subject notes** landed alongside, giving each
subject somewhere to keep context. The daily intention grew an evening
**reflection** twin — how the day went — and both bank into the `Journal`
at the midnight rollover, surfaced as the look-back on the stats page.
**Exports** let the data leave as files. The dbus pin was bumped again to
keep the patched protocol in step.

---

## Grades, reminders, onboarding (2026-06-15)

The grades tool expanded — more of the machinery a real degree needs — and
two long-standing gaps closed. **Deadline reminders**: `ReminderService`
fires a desktop alert once when an exam or due date first comes within
three days and again on the day itself, with the fired keys remembered in
state (and swept once past) so nothing notifies twice. **Onboarding**: a
first-run welcome so the app explains itself instead of opening cold.

---

## UI polish + settings restructure (2026-06-17)

A polish pass over the whole surface and a reorganisation of the settings
page into clearer groups. Tagged in spirit as the v1.3 line, though the app
had by now grown well past the original v1.3 scope.

> Housekeeping note: this commit also dropped the `docs/` folder by accident.
> The three docs were recovered from the previous commit and refreshed (see
> the next entry).

---

## The palette reaches the content (2026-06-17)

The command palette could jump to any page, a few actions, and subjects by
name — but not to the things you actually accumulate. Now it also searches
**todo tickets** (`#12 · title`), **flashcard decks**, and **journal
reflections** (date · snippet). Each row navigates and then *reveals*: a
ticket lands on the todo page filtered down to itself and expanded; a deck
opens; a reflection scrolls into view on the stats page with a brief amber
outline (`JournalRevealRequested` → `ContainerFromItem(...).BringIntoView()`
in the view's code-behind). Same keyboard-only flow, far more reach.

Alongside it, the drifted docs got an honest update: the README's feature
list and the broken `docs/` links, this journal, the architecture notes and
the roadmap — all brought back in line with where the app actually is.
